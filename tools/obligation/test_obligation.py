"""Validation suite for `_calculate_obligation`.
...
--expected  Ground truth. Columns: `crn` + any of `aff_emi`, `emi_unsec`,
            `aff_emi_topup`, `emi_unsec_topup`, `current_emi` (each compared
            numerically when present). Default: `sample_expected.csv`.
...
Note: there is no authoritative ground truth for obligation yet; `sample_expected.csv`
is seeded from a validated run and doubles as a regression guard.

Usage
-----
    python -m tools.obligation.test_obligation \
...
_METRICS = ("aff_emi", "emi_unsec", "aff_emi_topup", "emi_unsec_topup", "current_emi")
...
def run_suite(data_file, expected_file, *, delimiter="\t", tol=1.0):
    ...
    for e in _read_records(expected_file, ","):
        crn = str(e["crn"])
        out = _calculate_obligation(crn, rows=rows)
        checks, ok = [], True
        for m in _METRICS:
            raw = e.get(m)
            if raw in (None, ""):
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Validate bureau obligation against ground truth.")
    ap.add_argument("--data", default=BUREAU_DPD_FILE, help="tradeline data file")
    ap.add_argument("--expected", default=str(_DEFAULT_EXPECTED),
                    help="crn[,aff_emi][,emi_unsec][,aff_emi_topup][,emi_unsec_topup][,current_emi]")
